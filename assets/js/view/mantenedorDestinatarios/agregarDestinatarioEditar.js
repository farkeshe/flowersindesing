(function() {
  ...
  define(['backbone', 'text!tpl/mantenedorDestinatarios/adddestinatariopedido.html', 'assets/js/model/destinatario', 'assets/js/model/sector'], function(Backbone, TplViewAddCliente, DestinatariosModel, Sector) {
    var agregarDestinatarioEditar;
    return agregarDestinatarioEditar = (function(superClass) {
      extend(agregarDestinatarioEditar, superClass);

      function agregarDestinatarioEditar() {
        ...
        return agregarDestinatarioEditar.__super__.constructor.apply(this, arguments);
      }

      agregarDestinatarioEditar.prototype.initialize = function() {
        this.destinatarioModel = new DestinatariosModel();
        this.sector = new Sector();
        return this.init();
      };

      agregarDestinatarioEditar.prototype.init = function() {
        var log;
        ...
        $('#autoc2').val($('#autoc1Editar').val());
        $('#autoc3').val($('#DVEditar').val());
                  ...
                  return $.ajax({
                    ...
                    success: function(destino) {
                      ...
                      if (destino.error !== '202') {
                        ...
                        return Singleton.get().showExito('El destinatario ha sido guardado exitosamente.');
                      } else {
      ...
      agregarDestinatarioEditar.prototype.cargarRegionesEnMemoria = function(regiones) {
        return this.regiones = regiones;
      };

      agregarDestinatarioEditar.prototype.cargaProvinciaDefecto = function(p) {
        return this.provincias = p;
      };

      agregarDestinatarioEditar.prototype.cargaComunaDefecto = function(c) {
        return this.comunas = c;
      };

      agregarDestinatarioEditar.prototype.cargaProvincias = function(p) {
      ...
      agregarDestinatarioEditar.prototype.cargaComuna = function(p) {
        var i, iterator, len, option, results;
        option = '<option value="0" selected="selected">Seleccione...</option>';
        results = [];
        for (i = 0, len = p.length; i < len; i++) {
      ...
      agregarDestinatarioEditar.prototype.datosdestinatario = function() {
        var id;
      ...
      return agregarDestinatarioEditar;
